  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (resData.message.includes("duplicate")) {
          const message =
            "This post already exists. Please create only unique post.";
          setPublishErr(message);
          return;
        }
        ...
        >
          {loading ? (
            <div className="flex gap-2">
              <Spinner size="sm" /> <span>Publishing post...</span>
            </div>
          ) : (
            "Publish Post"
          )}
